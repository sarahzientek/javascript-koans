...
describe("About Applying What We Have Learnt", function() {
  ...
  it("should add all the natural numbers below 1000 that are multiples of 3 or 5 (functional)", function () {
    ...
    var sum = _.range(1000).reduce(function(f, t) {
      if (t % 3 === 0 || t % 5 === 0) {
        return f + t;
      }
      return f;
  ...
  /*********************************************************************************/
   it("should count the ingredient occurrence (imperative)", function () {
    var ingredientC = { "{ingredient name}": 0 };

    for (i = 0; i < products.length; i+=1) {
        for (j = 0; j < products[i].ingredients.length; j+=1) {
            ingredientC[products[i].ingredients[j]] = (ingredientC[products[i].ingredients[j]] || 0) + 1;
        }
    }

    expect(ingredientC['mushrooms']).toBe(2);
  });

  it("should count the ingredient occurrence (functional)", function () {
    var ingredientC = { "{ingredient name}": 0 };

    ingredientC = _(products).chain()
